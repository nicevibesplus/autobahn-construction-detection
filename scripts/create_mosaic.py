...
def run_mosaic_extraction(input_dir, output_dir, roads_gpkg, grid_split, road_search_buffer_m, master_mosaic_path):
    ...
    if master_mosaic_path.exists():
        print(f"\n[{time.strftime('%H:%M:%S')}] [INFO] Master-Bild existiert bereits: {master_mosaic_path.name}")
        print(f"[{time.strftime('%H:%M:%S')}] Generierung übersprungen. Nutze existierende Datei.")
    ...
    # Added tqdm progress bar for iterating over JP2 files during bounds collection
    ...
    # Added tqdm progress bar for iterating through JP2 files during tile extraction
    ...
    # Added tqdm progress bar for opening temporary tiles to mosaic
    ...
    # Added tqdm progress bar for closing sources after merging
